...
// 批量删除章节数据
const batchRemoveChapterListSync = (chapterIds) => ({
  type: BATCH_REMOVE_CHAPTER_LIST,
  data: chapterIds,
});

export const batchRemoveChapterList = (chapterIds) => {
  return (dispatch) => {
    return reqBatchRemoveChapterList(chapterIds).then((response) => {
      dispatch(batchRemoveChapterListSync(chapterIds));
      return chapterIds;
    });
  };
};
